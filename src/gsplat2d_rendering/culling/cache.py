...
def load_or_build_octree(
    ...
    above."""
    # Cache-decision-level messages only, below -- load_octree/build_octree/
    # save_octree already report their own leaf counts/timing themselves
    # (see culling/octree.py), so this function doesn't repeat those figures.
    ...
    if cache_path.is_file():
        ...
        if not needs_lod_rebuild:
            return octree
        if not build_index:
            warning(__name__, f"Cached index at {cache_path} has no LOD proxies -- "
                    "pass build_index=True to regenerate it with compute_lod enabled; "
                    "LOD disabled for this run")
            return octree
        info(__name__, f"Cached index at {cache_path} has no LOD proxies -- "
        ...
        warning(__name__, f"No octree index at {cache_path} and build_index=False -- culling disabled")
        return None
    ...
    if compute_lod:
        ...
        info(__name__, f"Building LOD proxies ({len(octree.node_aabbs):,} leaves) ...")
